...
export class MultiSelect implements OnInit,AfterViewInit,AfterViewChecked,DoCheck,OnDestroy,ControlValueAccessor {
    ...
    onItemClick(event, value, option) {
        ...
        let valueTemp = [].concat(this.value);
        let selectionIndex = this.findSelectionIndex(value);
        if(selectionIndex != -1)
          valueTemp = valueTemp.filter((val,i) => i!=selectionIndex);
        else
          valueTemp = [...valueTemp||[],value];
          ...
          this.value = [];
            ...
            if(opts) {
                ...
                for(let i = 0; i < opts.length; i++) {
                    this.value.push(opts[i].value || opts[i].id);
                }
            }
        }
    ...
    onFilter(event) {
        ...
        for(let i = 0; i < this.options.length; i++) {
            let option = this.options[i];
            if((option.label && option.label.toLowerCase().indexOf(this.filterValue.toLowerCase()) > -1)||(option.name && option.name.toLowerCase().indexOf(this.filterValue.toLowerCase()) > -1)) {
    ...
    isItemVisible(option: SelectItem): boolean {
        if(this.filterValue && this.filterValue.trim().length) {
            for(let i = 0; i < this.visibleOptions.length; i++) {
                if(this.visibleOptions[i].value == option.value || this.visibleOptions[i].name == option.name) {
                    return true;
                }
            }
        }
    ...
    getVisibleOptions(): SelectItem[] {
        if(this.filterValue && this.filterValue.trim().length) {
            ...
            for(let i = 0; i < this.options.length; i++) {
                let option = this.options[i];
                if(option.label.toLowerCase().includes(this.filterValue.toLowerCase()) || option.name.toLowerCase().includes(this.filterValue.toLowerCase())) {
                    items.push(option);
                }
            }
            return items;
